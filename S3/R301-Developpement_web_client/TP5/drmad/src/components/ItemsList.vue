<template>
  <div class="list-wrapper">
    ...
    <hr />
    <h1>Viruses</h1>
    <CheckedList @list-button-clicked="addAllToCart($event)" @item-button-clicked="addToCart($event)"
...
export default {
  ...
  computed: {
    ...
    checked() {
      let checked = [];
      let filteredIndexes = this.filterViruses.map(v => this.viruses.indexOf(v));
      for (let i = 0; i < this.filterViruses.length; i++) {
        checked.push(this.selected.includes(filteredIndexes[i]));
      }
      return checked;
    }
  ...
  methods: {
    ...
    async addAllToCart(data) {
      for ( const item of data) {
        const virus = this.filterViruses[item.id]
        const amount = item.amount
        await this.addItemToBasket({ virus, amount })
      }
    },
    addToCart(data) {
    ...
    changeAllSelected() {
      ...
      if (this.globalChecked) {
        for (let index of filteredIndexes) {
